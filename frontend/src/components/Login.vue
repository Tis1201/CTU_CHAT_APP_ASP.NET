...
<script setup>
import { ref } from 'vue';
import makeAuthenticationService from '@/service/authentication.service.js';
import { useRouter } from 'vue-router';

const router = useRouter();
// Reactive state to track the active form
...
const handleLogin = async () => {
  ...
  try {
    ...
    const { customer_id, full_name, email, role } = payload;
...
.container {
  ...
  overflow: hidden;
  width: 100%;
  max-width: 768px; /* Limits the maximum width */
  min-height: 480px;
  display: flex;
...
.toggle {
  background-image: url('../../public/img/Peace.gif');
  height: 100%;
  color: #f7fbff;
  position: relative;
  left: -100%;
